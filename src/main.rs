...
impl Execx {
    ...
    async fn execit(db_value: String, bk_value: String, ref_value: String, rows_num: u64, outdir: String, excludefile: String, tstrun: bool, hourmust: bool, datemust: bool, tx_send: mpsc::UnboundedSender<String>,) -> Result<Execx, Error> {
     ...
     let mut vecfilename: Vec<String> = Vec::new();
     let mut vecdirname: Vec<String> = Vec::new();
     let mut vecfiledate: Vec<String> = Vec::new();
     let mut vecmd5sum: Vec<String> = Vec::new();
     let mut vecfilesize: Vec<u64> = Vec::new();
     let mut vecrowid: Vec<u64> = Vec::new();
...
//            get list of reference name entries
                   ...
                   Ok(mut stmt) => {
                       match stmt.query_map(&[(":ref", &ref_value)], |row| {
                          ...
                          Ok(bk_iter) => {
                              for bk in bk_iter {
                                   let bki = bk.unwrap();
                                   vecfilename.push(bki.filename.to_string());
                                   vecdirname.push(bki.dirname.to_string());
                                   vecfiledate.push(bki.filedate.to_string());
                                   vecfilesize.push(bki.filesize);
                                   vecrowid.push(bki.rowid);
                                   if bki.md5sum == None {
                                       vecmd5sum.push("None".to_string());
                                   } else {
                                       vecmd5sum.push(bki.md5sum.as_ref().unwrap().to_string());
                                   }
                              }
                          }
                          Err(err) => {
                              errstring = format!("error of {} sql call for reference list a ", err);
                              errcode = 4;
                              bolok = false;   
                          }
                       }
     ...
     if bolok {
         ...
         writeln!(&mut excludeop, "testrun: {}  bklist: {}", tstrun, bk_value).unwrap();
         let mut noop = File::create(noentriesout).unwrap();
         writeln!(&mut noop, "testrun: {}  bklist: {}", tstrun, bk_value).unwrap();
         let mut errop = File::create(errout).unwrap();
         writeln!(&mut errop, "testrun: {}  bklist: {}", tstrun, bk_value).unwrap();
...
//   check if excluded
                  ...
                  if bnotex {
                      let mut entryinx = 0;
                      let mut foundfildir = false;
                      for dbfilenmx in vecfilename.iter() {
                           let dbfilename1: String;
                           if dbfilenmx.contains("&apos;") {
                               dbfilename1 = dbfilenmx.replace("&apos;", "'");
                           } else {
                               dbfilename1 = dbfilenmx.to_string();
                           }
                           let dbfilename: String;
                           if dbfilename1.contains("&amp;") {
                               dbfilename = dbfilename1.replace("&amp;", "&");
                           } else {
                               dbfilename = dbfilename1.to_string();
                           }
                           if dbfilename == inptfilenm {
                               let mut boldir = false;
                               let dbdirnm1: String;
                               if vecdirname[entryinx].contains("&apos;") {
                                   dbdirnm1 = vecdirname[entryinx].replace("&apos;", "'");
                               } else {
                                   dbdirnm1 = vecdirname[entryinx].to_string();
                               }
                               let dbdirnm0: String;
                               if dbdirnm1.contains("&amp;") {
                                   dbdirnm0 = dbdirnm1.replace("&amp;", "&");
                               } else {
                                   dbdirnm0 = dbdirnm1.to_string();
                               }
                               let inptdirnmshort: String;
                               if inptdirnm.len() > dbdirnm0.len() {
                                   inptdirnmshort = inptdirnm[(inptdirnm.len() - dbdirnm0.len())..].to_string();
                               } else {
                                   inptdirnmshort = inptdirnm.clone();
                               }            
                               if dbdirnm0 != inptdirnmshort{
                                   let dbdirnm1 = dbdirnm0.replace("\\", "/");
                                   if dbdirnm1 == inptdirnmshort {
                                       boldir = true;
                                   } else if dbdirnm1.len() < 2 && inptdirnmshort.len() < 2 {
                                       boldir = true;
                                   } else {
                                       let dbdirnm2 = &("/".to_owned() + &dbdirnm1);
                                       if dbdirnm2 == &inptdirnmshort {
                                           boldir = true;
                                       }
                                   }
                               } else {
                               ...
                               if !boldir {
                                   writeln!(&mut errop, "for file {} directories do not match db:{} input:{}", dbfilename, dbdirnm0, inptdirnmshort).unwrap();
                               } else {
                                   let dbfilesize = vecfilesize[entryinx];
                                   ...
                                   if inptsizi64 == -9999 {
                                       ...
                                       if dbfilesize != inptsizi64 as u64 {
                                           ...
                                           let mut dbtemdate = String::new();
                                           let mut dbfiledate = &String::new();
                                           let mut dbdatenew = false;
                                           if !vecfiledate[entryinx].contains("/") {
                                               dbfiledate = &vecfiledate[entryinx];
                                           } else {
                                               let datexvec: Vec<&str> = vecfiledate[entryinx].split("/").collect();
                                               let datexlen = datexvec.len();
                                               if datexlen != 3 {
                                                   errstring = format!("*invalid date of {} for file {} {} of rowid of {}", vecfiledate[entryinx], vecdirname[entryinx], vecfilename[entryinx], vecrowid[entryinx]);
                                                   bolok = false;
                                               } else { 
                                                  let mut dateyr1: i64 = 0;
                                                  let mut datemo1: i64 = 0;
                                                  let mut dateday1: i64 = 0;
                                                  for indl in 0..datexlen {
                                                       let date_int: i32 = datexvec[indl].parse().unwrap_or(-9999);
                                                       if date_int == -9999 {
                                                           bolok = false;
                                                           errstring = format!("**invalid date of {} for file {} {} of rowid of {}", vecfiledate[entryinx], vecdirname[entryinx], vecfilename[entryinx], vecrowid[entryinx]);
                                                           ...
                                                           match indl {
                                                            0 => datemo1 = date_int as i64,
                                                            1 => dateday1 = date_int as i64,
                                                            2 => dateyr1 = date_int as i64,
                                                            _ => {errstring = format!("***invalid date of {} for file {} {} of rowid of {}", vecfiledate[entryinx], vecdirname[entryinx], vecfilename[entryinx], vecrowid[entryinx]);
                                                                  bolok = false;},
                                                           }
                                                      }
                                                  }
                                                  if bolok {
                                                      let datexx = Local.with_ymd_and_hms(dateyr1 as i32, datemo1 as u32, dateday1 as u32, 0, 0, 0);
                                                      if datexx == LocalResult::None {
                                                          errstring = format!("****invalid date of {} for file {} {} of rowid of {}", vecfiledate[entryinx], vecdirname[entryinx], vecfilename[entryinx], vecrowid[entryinx]);
                                                          bolok = false;
                                                      } else {
                                                          dbtemdate = format!("{:04}-{:02}-{:02} 00:00:00.000", dateyr1, datemo1, dateday1 );
                                                          dbfiledate = &dbtemdate;
                                                          dbdatenew = true;
                                                      }
                                                  }
                                               }
                                           ...
                                           if dbfiledate == vecline[2] {
                                               if vecmd5sum[entryinx] == "None" {
                                                   writeln!(&mut errop, "md5sum is null for {}", inptfilenm).unwrap();
                                                   if !tstrun {
                                                       let strrow = format!("{}", vecrowid[entryinx]);
                                                       ...
                                                       match conn.prepare("UPDATE blubackup SET md5sum = :md5 WHERE rowid = :rwd") {
                                                               ...
                                                               match upit.query_map(&[(":md5", &vecline[5]),(":rwd", &strrowx)], |row| {
                                                                 ...
                                                                 Ok(upit_iter) => {
                                                                    for uupit in upit_iter {
                                                                         ...
                                                                         writeln!(&mut errop, "update listing output {:?}, row {}", uuupit.name, vecrowid[entryinx]).unwrap();
                                                                    }
                                                                 }
                                                                 Err(err) => {
                                                                     errstring = format!("error of {} sql call for update a ", err);
                                                                     errcode = 6;
                                                                     bolok = false;
                                                                     break;  
                                                                 }
                                                          ...
                                                          Err(err) => {
                                                              errstring = format!("error of {} sql call for update b ", err);
                                                              errcode = 6;
                                                              bolok = false;
                                                              break;  
                                                          }
                                                       ...
                                                       writeln!(&mut errop, "test run - update {}, db date {} row {}", inptfilenm, dbfiledate, vecrowid[entryinx]).unwrap();
                                                   }
                                               } else {
                                                   let strbkimd5sum = &vecmd5sum[entryinx];
          ...
          // if filedate not equal
                                               ...
                                               if vecmd5sum[entryinx] == "None" {
                                                   if bolonlyhr {
                                                       if !hourmust {
                                                           ...
                                                           if !tstrun {
                                                               ...
                                                               match conn.prepare("UPDATE blubackup SET md5sum = :md5 WHERE rowid = :rwd") {
                                                                  Ok (mut upit) => {
                                                                       match upit.query_map(&[(":md5", &vecline[5]),(":rwd", &strrowx)], |row| {
                                                                          ...
                                                                          Err(err) => {
                                                                             errstring = format!("error of {} sql call for update c ", err);
                                                                             errcode = 7;
                                                                             bolok = false;
                                                                             break;  
                                                                          }
                                                                  ...
                                                                  Err(err) => {
                                                                       errstring = format!("error of {} sql call for update d ", err);
                                                                       errcode = 8;
                                                                       bolok = false;
                                                                       break;  
                                                                  }
                                                               ...
                                                               writeln!(&mut errop, "test run - update {}, db date {} daterow {}",inptfilenm, dbfiledate, vecrowid[entryinx]).unwrap();
                                                           }
                                                       } else {
                                                   ...
                                                   if vecmd5sum[entryinx] == vecline[5].to_string() {
                                                       if bolonlyhr {
                      ...
                      if !foundfildir { 
                          writeln!(&mut noop, "did not find in database dir:{} file:{}",inptdirnm, inptfilenm).unwrap();
                      } else {
                          numeq1 = numeq1 + 1;
                      }
                  }
                  ...
                  let msgx = format!("Progress|{}|{}| elapsed time {:.1} mins at {}", linenumx, rows_num, minsx, datexx.format("%H:%M:%S"));
         ...
         if bolok {
             errstring = format!("Completed {} of {} files with output in suffix {:02}", numeq1, linenumx, outseq);
         }
     } 
    Ok(Execx {
            errcd: errcode,
